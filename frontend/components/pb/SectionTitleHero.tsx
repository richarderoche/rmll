...
export default function SectionTitleHero({
  ...
  const {rowWidth, titleMode, title, subtitle, image} = section
...
export const LabeledHeading = ({
  ...
  return (
    <div className="flex flex-col gap-y-gut">
      {label && <TopTag className={cn('ts-h5', labelColorClass)}>{label}</TopTag>}
      {heading && <HeadingTag className={cn(headingTsClass, 'text-balance')}>{heading}</HeadingTag>}
    </div>
  )
}
